<?php
			...
			if ($ins_rentals){
				...
				for ($c = 0; $c < $n_details; $c++){
					if (!validateNumbers(1, 4, $p_mlt[$c]) || !validateNumbers(1, 6, $p_total_meter[$c]) || (!validateNumbers(1, 6, $p_total_meter[$c]) && $p_page_distinction)){
						var_dump($p_mlt[$c]);
						var_dump($p_total_meter[$c]);
						var_dump($p_color_meter[$c]);
						die();
						$title = "Erro";
						...
						if ($ins_printers){
							...
							if($upd_printers){
								$title = "Sucesso";
								$message = "Aluguel inserido com sucesso.";
							} else {
								$title = "Erro";
				...
				$title = "Erro";
				$message = "Erro na inserção de aluguel.";
			}
		} else {
			$title = "Erro";
	...
	<div class="center_box">
		...
		<?php echo $message; ?>
		<br/><a href="?folder=rentals/&file=fmins_rentals&ext=php"><img height="15" src="../layout/images/back.png"> Voltar</a>
	</div>
